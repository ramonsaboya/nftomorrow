...
test('compact USD message converts the total and shows the matching fresh SOL rate', async () => {
  ...
  assert.match(text, /^Medallion: 450\.75 USD \(3 SOL\)/);
  assert.match(text, /The Reflection of Love: 1 SOL/);
  assert.match(text, /SOL -> USD: 150\.25 USD/);
  assert.match(text, /Date checked: .*19:05 BST/);
  ...
  assert.match(stale, /Medallion: USD unavailable \(3 SOL\)/);
  assert.match(stale, /SOL -> USD: unavailable/);
