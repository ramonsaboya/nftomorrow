...
- Node 22.23.2 on macOS; syntax checks and 22 Node test cases pass, including delivery bookkeeping, QR session restart regressions and bounded CLI shutdown with output flushing.
...
## Pending policy confirmation and implementation

Threshold crossing/recovery, combined alerts, daily suppression, London DST, hourly scheduling, persistent alert state, complete reconnect orchestration, end-to-end duplicate prevention, and the systemd unit are not yet implemented or tested.

## Not exercised live

Actual recipient notifications; Healthchecks email; Linux VPS installation, reboot or restoration; and a full daily cycle. Simulated tests do not prove these behaviors. The price snapshot is a one-time fetch, not evidence of continuous monitoring.
