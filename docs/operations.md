...
For current NFT prices, tag Dobby and type `status` from another WhatsApp account in the configured group while the service is connected. It fetches fresh prices and replies in that group. Standalone `/status` is ignored in groups; send `/status` in a 1:1 chat with Dobby for a private reply. Allow one minute between requests. This is a price command; use the administrator commands below for process health. Commands sent while offline are not replayed after reconnect.
